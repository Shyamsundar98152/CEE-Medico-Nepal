...
const modalErrorMessage = document.getElementById('modalErrorMessage'); // Get modal error message element
...
window.startQuiz = async function (name, email, onSuccessCallback) {
    if (!name) {
        modalErrorMessage.textContent = 'Name is required to start the quiz.';
        return;
    }

    modalErrorMessage.textContent = 'Checking user...';

    // Check if the user (by name) has already taken the quiz
    const exists = await checkUserExist(name);
    if (exists) {
        modalErrorMessage.textContent = `User '${name}' has already taken the quiz. Please use a different name or view the leaderboard.`;
        return;
    }
